...
def download_data(url:str, fname:[Path or str]=None, data:bool=True, ext:str='.tgz') -> Path:
    ...
    fname = Path( fname if fname is not None else _url2tgz(url, data, ext=ext) )
...
def untar_data(url:str, fname:[Path or str]=None, dest:[Path or str]=None, data=True, force_download=False) -> Path:
    "Download `url` to `fname` if it doesn't exist, and un-tgz to folder `dest`."
    dest = url2path(url, data) if dest is None else Path(dest)/url2name(url)
    fname = Path( fname if fname is not None else _url2tgz(url, data) )
